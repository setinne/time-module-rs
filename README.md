...
int main() {
    ...
    // 5. 获取格式化时间字符串（需要手动释放内存）
    const char* timeStr = GetFormattedTime();
    printf("格式化时间: %s\n", timeStr);
    FreeString((char*)timeStr);  // 重要：必须释放！
...
print(f"本地时间: {ft.year}-{ft.month:02d}-{ft.day:02d} {ft.hour:02d}:{ft.minute:02d}:{ft.second:02d}.{ft.ms:03d}")

# 获取格式化时间字符串（需要释放内存）
time_str = dll.api_GetFormattedTime()
print(f"格式化时间: {time_str.decode('utf-8')}")
dll.api_FreeString(time_str)  # 释放内存
```

### Python 示例（ctypes - 纳秒精度）
...
# 定义纳秒精度时间结构体
class FullTimeNs(Structure):
    _fields_ = [
        ("year", c_int), ("month", c_int), ("day", c_int),
        ("hour", c_int), ("minute", c_int), ("second", c_int),
        ("ns", c_int)      # 纳秒（0-999,999,999）
    ]
...
# 设置函数返回类型和参数类型
dll.api_GetLocalTimeNs.restype = FullTimeNs
dll.api_SetCalendarType.argtypes = [c_int]
dll.api_GetCalendarType.restype = c_int

# 设置历法为公历（0=公历，1=儒略历）
dll.api_SetCalendarType(0)

# 获取纳秒精度时间
ft = dll.api_GetLocalTimeNs()
print(f"纳秒时间: {ft.year}-{ft.month:02d}-{ft.day:02d} {ft.hour:02d}:{ft.minute:02d}:{ft.second:02d}.{ft.ns:09d}")
```
...
class TimeModule
{
    ...
    [DllImport("time_module.dll", CallingConvention = CallingConvention.Cdecl)]
    public static extern FullTimeNs api_GetLocalTimeNs();

    [DllImport("time_module.dll", CallingConvention = CallingConvention.Cdecl)]
    public static extern void api_SetCalendarType(int calType);

    [DllImport("time_module.dll", CallingConvention = CallingConvention.Cdecl)]
    public static extern int api_GetCalendarType();
    ...
    static void Main()
    {
        ...
        // 设置历法为公历（0=公历，1=儒略历）
        api_SetCalendarType(0);
        Console.WriteLine($"当前历法: {(api_GetCalendarType() == 0 ? "公历" : "儒略历")}");

        // 获取微秒精度时间
        FullTime ft = api_GetLocalTime();
        Console.WriteLine($"本地时间: {ft.year}-{ft.month:D2}-{ft.day:D2} {ft.hour:D2}:{ft.minute:D2}:{ft.second:D2}.{ft.ms:D3}");

        // 获取纳秒精度时间
        FullTimeNs ftNs = api_GetLocalTimeNs();
        Console.WriteLine($"纳秒时间: {ftNs.year}-{ftNs.month:D2}-{ftNs.day:D2} {ftNs.hour:D2}:{ftNs.minute:D2}:{ftNs.second:D2}.{ftNs.ns:D9}");

        // 获取格式化时间字符串（需要释放内存）
        IntPtr ptr = api_GetFormattedTime();
        string timeStr = Marshal.PtrToStringAnsi(ptr);
        Console.WriteLine($"格式化时间: {timeStr}");
        api_FreeString(ptr);  // 重要：必须释放！
    }
}
```
...
| `api_GetFormattedTime()` | `const char*` | 获取格式化的时间字符串（需调用 `api_FreeString` 释放） |
...
| `api_SetTimezoneByName()` | `int` | 通过名称设置时区（如 "UTC+8"），返回 0 成功 |
| `api_SetTimezoneByLocation()` | `int` | 通过经纬度设置时区，返回 0 成功 |
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_GetVersion()` | `int` | 获取 DLL 版本号（如 0x000207 表示 v0.2.7） |
| `api_GetVersionString()` | `const char*` | 获取版本号字符串（无需释放） |
| `api_GetErrorString()` | `const char*` | 获取错误码对应的描述文字（无需释放） |
...
- 多个偏移秒：系统会根据经度自动选择（美国、加拿大、俄罗斯、澳大利亚、巴西、印尼、哈萨克斯坦等大国已内置分区算法，其他国家退回到经纬度计算）
...
- `api_GetFormattedTime()` 返回的字符串需要调用 `api_FreeString()` 释放，否则会造成内存泄漏
- `api_GetVersionString()` 和 `api_GetErrorString()` 返回静态字符串，**无需释放**
- 年份范围：支持公元前 4713 年至公元后 9999 年（公历）/ 儒略历
- 历法说明：默认使用公历，可通过 `api_SetCalendarType()` 切换到儒略历
- 纳秒精度：`api_GetLocalTimeNs()` 提供更高精度，适用于需要精确计时的场景
- 线程安全：所有 API 函数都是线程安全的
- DLL 卸载：在卸载 DLL 前建议调用 `api_Shutdown()` 通知后台线程退出
...
**Q4: 网络时间不可用怎么办？**  
A: 检查防火墙是否允许 UDP 123 端口出站，或手动调用 `api_ForceResyncEx()` 强制同步。

**Q5: 时区偏移超出范围会怎样？**  
A: 设置函数会返回错误码 1（InvalidParam），时区偏移保持原值不变。有效范围：-50400 ~ 50400 秒（UTC-14 ~ UTC+14）。

**Q6: 夏令时如何工作？**  
A: 支持两种后端：① 内建规则表（`dst_rules.txt`），② Windows 系统 API。可通过 `api_SetDSTBackend()` 切换。默认使用规则表。

**Q7: 支持 2038 年之后的时间吗？**  
A: 完全支持。内部使用 64 位整数时间戳，可正确处理到 9999 年。

**Q8: 支持哪些历法？**  
A: 支持公历（格里高利历）和儒略历，可通过 `api_SetCalendarType()` 切换。默认使用公历。

**Q9: 时间精度是多少？**  
A: 
- `api_GetLocalTime()` 返回微秒精度（us）
- `api_GetLocalTimeNs()` 返回纳秒精度（ns）

**Q10: 需要手动释放内存吗？**  
A: 
- `api_GetFormattedTime()` 返回的字符串必须调用 `api_FreeString()` 释放
- `api_GetVersionString()` 和 `api_GetErrorString()` 返回静态字符串，无需释放
- 其他返回结构体的 API 不需要释放
...
| [time_module_normal_x86.dll](https://github.com/setinne/time-module-rs/releases/latest) | 32位 | ~1,046 KB |
| [time_module_upx_x64.dll](https://github.com/setinne/time-module-rs/releases/latest) | 64位 (UPX) | ~365 KB |
| [time_module_upx_x86.dll](https://github.com/setinne/time-module-rs/releases/latest) | 32位 (UPX) | ~394 KB |

> 推荐使用 UPX 压缩版本以减小体积。Windows Vista 32 位用户若 UPX 压缩版不可用，请使用原始版本。
