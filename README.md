...
| `api_GetFormattedTimeBuf()` | `int` | **安全版本**：写入调用者提供的缓冲区。可传 `buf=NULL, size=0` 查询所需长度 |
...
| `api_GetBaseOffsetByLocation()` | `int` | 获取基础时区偏移（不含 DST），失败返回 -1 |
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_GetWeekday()` | `int` | 获取星期几（0=星期日, 1=星期一, ..., 6=星期六） |
| `api_GetWeekdayISO()` | `int` | 获取星期几（1=星期一, 2=星期二, ..., 7=星期日） |
| `api_GetWeekdayName()` | `const char*` | 获取英文星期名称（**需调用 `api_FreeString` 释放**） |
| `api_GetWeekdayNameZh()` | `const char*` | 获取中文星期名称（**需调用 `api_FreeString` 释放**） |
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_IsLeapYear()` | `bool` | 判断是否为闰年 |
| `api_IsLeapYearEx()` | `int` | 判断是否为闰年（返回 1/0） |
| `api_DayOfYear()` | `int` | 获取指定日期在一年中的第几天（1-366） |
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_GetVersion()` | `int` | 获取 DLL 版本号（如 0x00020a 表示 v0.2.10） |
...
- **内存释放**：所有返回 `const char*` 的 API（`api_GetFormattedTime`、`api_GetVersionString`、`api_GetErrorString`、`api_GetWeekdayName`、`api_GetWeekdayNameZh`）均返回动态分配的内存，**必须**调用 `api_FreeString` 释放。推荐使用 `api_GetFormattedTimeBuf` 避免手动内存管理。
...
- **后台线程退出**：在卸载 DLL 前建议调用 `api_Shutdown()` 通知后台线程退出。
- **年份范围**：支持公元前 4713 年至公元后 9999 年（公历 proleptic Gregorian）。
