...
- ✅ **多实例支持** - 通过句柄模式可创建多个独立配置的时间模块
- ✅ **异步 NTP 同步**  - 非阻塞强制同步，回调通知结果
...
int main() {
    ...
    if (!pfn_SetTimezoneOffset || !pfn_GetFormattedTimeBuf || !pfn_Shutdown) {
        printf("获取函数地址失败\n");
        FreeLibrary(dll);
        return 1;
    }
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_GetVersion()` | `int` | 获取 DLL 版本号（如 0x000211 表示 v0.2.17） |
...
| [time_module_x64.lib](https://github.com/setinne/time-module-rs/releases/latest) | 64位导入库 (MSVC) | 1.6 KB |
| [time_module_x86.lib](https://github.com/setinne/time-module-rs/releases/latest) | 32位导入库 (MSVC) | 2.4 KB |
