...
以下是一个最小示例，演示安全缓冲区接口和 `Ex` 系列函数的使用：
...
int main() {
    ...
    if (!pfn_SetTimezoneOffset || !pfn_GetFormattedTimeBuf || !pfn_IsNTPSyncedEx || !pfn_Shutdown) {
        printf("获取函数地址失败\n");
        FreeLibrary(dll);
        return 1;
    }
    ...
    // 检查 NTP 状态（返回 1 表示已同步）
    printf("NTP 已同步: %s\n", pfn_IsNTPSyncedEx() ? "是" : "否");
...
完整的跨语言示例（C, C++, C#, Python, Go, Rust, Java, Delphi, VB.NET, Node.js）请查看项目仓库中的 [`examples/`](https://github.com/setinne/time-module-rs/tree/main/examples) 目录。
...
| 函数名 | 返回值 | 说明 |
|--------|--------|------|
| `api_GetVersion()` | `int` | 获取 DLL 版本号（如 0x00020f 表示 v0.2.15） |
...
- **必须调用 `api_Shutdown()`**：在卸载 DLL 前**必须**调用此函数以通知后台线程退出，否则可能导致进程崩溃。
...
- **日志回调**：可通过 `api_SetLogCallback` 注册回调，回调函数接收文件名、行号、时间戳和日志消息，便于生产环境监控。
...
| 文件 | 架构 | 大小（约） |
|------|------|------------|
| [time_module_normal_x64.dll](https://github.com/setinne/time-module-rs/releases/latest) | 64位 | 1,082 KB |
| [time_module_normal_x86.dll](https://github.com/setinne/time-module-rs/releases/latest) | 32位 | 1,085 KB |
| [time_module_upx_x64.dll](https://github.com/setinne/time-module-rs/releases/latest) | 64位 (UPX) | 380 KB |
| [time_module_upx_x86.dll](https://github.com/setinne/time-module-rs/releases/latest) | 32位 (UPX) | 410 KB |
