...
mod v0_2_19_tests {
    ...
    fn test_timezone_offset_range() {
        // 测试有效范围内的偏移 (v0.2.19: 范围扩大到 UTC+14 = 50400)
        let valid_offsets = vec![-43200, -28800, 0, 28800, 43200, 46800, 50400];
        for &offset in &valid_offsets {
            assert!(offset >= MIN_VALID_OFFSET && offset <= MAX_VALID_OFFSET,
                    "Offset {} should be valid (MIN={}, MAX={})", 
                    offset, MIN_VALID_OFFSET, MAX_VALID_OFFSET);
        }

        // 测试超出范围的偏移
        let invalid_offsets = vec![-50400, -50000, 54000, 57600];
        for &offset in &invalid_offsets {
            assert!(offset < MIN_VALID_OFFSET || offset > MAX_VALID_OFFSET,
                    "Offset {} should be invalid (MIN={}, MAX={})", 
                    offset, MIN_VALID_OFFSET, MAX_VALID_OFFSET);
        }
    }
