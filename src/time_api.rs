...
use crate::time::{handle, config, tz_country, tz_name, dst};
...
pub extern "C" fn api_SetTimezoneByLocation(lon: f64, lat: f64, code: *const c_char) -> i32 {
    let result = (|| {
        ...
        let base_offset = tz_country::offset_from_location(lon, lat, country)?;
        
        // 获取本地时间用于 DST 判断
        let (sec, us) = handle::get_calibrated_local_time();
        let local = handle::formatting::utc_to_fulltime(sec, us, base_offset);
